...
- [x] P.1 Секция: моно-подпись «What's next?» / «Что дальше?», крупный заголовок «Get In Touch» / «Свяжитесь со мной», 1-2 строки текста (открыт к предложениям, удалённо), большая ghost-кнопка `Say hello` → mailto, рядом SocialIcons. Центрировано, много воздуха.
- [x] P.2 Проверка, сборка, коммит.

### Задача Q: Облако тегов навыков

**Files:** Skills.astro, новый src/scripts/skill-cloud.js.

- [x] Q.1 Canvas-сфера тегов (vanilla, без библиотек, ~120 строк): теги на сфере (fibonacci-распределение), автовращение + доворот за курсором, размер/яркость по весу (3 уровня: Unity/C#/ECS·DOTS/Multiplayer крупные; средние — Jobs/Burst/оптимизация/архитектура/DI/тесты; мелкие — прочее), глубина = прозрачность. Высота ~380px.
- [x] Q.2 Fallback: под canvas остаются текущие статичные теги, скрытые при активном JS (`html.js .skills-static { display:none }`); reduced-motion → статичные теги.
- [x] Q.3 Проверка (вращение, вес, курсор), сборка, коммит.
...
- [x] S.1 Убрать букву «ё» ВЕЗДЕ в RU-текстах (е вместо ё), включая ui.ts и md.
- [x] S.2 Редакторская вычитка: единый тон, компоновка абзацев, паразитные повторы, длина taglines. Ничего не выдумывать.
- [x] S.3 Проверка, сборка, коммит.
