...
class SkillCloud {
  ...
  constructor(canvas, tags, onFirstDraw) {
    ...
    // (canvas.offsetWidth/Height === 0, если канвас на момент вызова еще
  ...
  resize() {
    ...
    // в CSS — берем той же константой (CANVAS_HEIGHT), не завязываясь на бокс
    // самого канваса.
    const parent = c.parentElement;
    const w = parent ? parent.clientWidth : c.offsetWidth;
    const h = CANVAS_HEIGHT;
    // Если ширины все еще нет (например, секция сама пока не имеет раскладки —
  ...
  _observeResize() {
    ...
    // {0,0}, и ResizeObserver на нем ничего полезного не даст.
  ...
  _tick(now) {
    ...
    if (!document.hidden) {
      // fix (P0, self-heal): если канвас еще не измерен валидно (size 0 —
...
(function init() {
  ...
  // canvas.offsetWidth/offsetHeight; пока канвас еще display:none (CSS-дефолт
  ...
  //
  // Фикс: канвас остается display:none (и список — видимым) ровно до первого
